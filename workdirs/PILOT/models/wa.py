...
class Learner(BaseLearner):
    ...
    def _load_checkpoint(self, train_loader, test_loader, optimizer, scheduler):
        ckp_path = os.path.join(
            self.args["ckp_dir"],
            f"{self.args['model_name']}-{self.args['backbone_type']}-init-seed{self.args['seed']}.pth",
        )
        if os.path.exists(ckp_path):
            ...
            def add_module_prefix(key):
                return f"module.{key}" if not key.startswith("module.") else key
    ...
    def create_loss_fn(self, inputs, targets, model=None):
        ...
        kd_lambda = self._known_classes / self._total_classes if self._total_classes > 0 else 0.0
        ...
        def loss_fn():
            ...
            return logits, [(1 - kd_lambda) * loss_clf, kd_lambda * loss_kd]
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for batch_idx, (_, inputs, targets) in enumerate(train_loader):
                ...
                if self._conflict_stats_enabled() and batch_idx < self._conflict_probe_batches():
...
def _KD_loss(pred, soft, temperature):
    pred = torch.log_softmax(pred / temperature, dim=1)
    soft = torch.softmax(soft / temperature, dim=1)
