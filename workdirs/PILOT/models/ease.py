...
class Learner(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task == 0 or self.init_cls == self.inc:
            base_optimizer = self.get_optimizer(lr=self.args["init_lr"])
            optimizer = get_infty_optimizer(params=self._network.parameters(), base_optimizer=base_optimizer, model=self._network, args=self.args)
            ...
            base_optimizer = self.get_optimizer(lr=self.args["later_lr"])
    ...
    def create_loss_fn(self, inputs, targets, model=None):
        ...
        if model is None:
            model = self._network
        def loss_fn():
            logits = model(inputs, test=False)["logits"]
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if self.args["inftyopt"] == "forward_grad":  
                    loss_fn, batch = self.create_jvp_loss_fn(inputs, aux_targets)
                    optimizer.set_closure(loss_fn, batch)
                else:
                    loss_fn = self.create_loss_fn(inputs, aux_targets)
